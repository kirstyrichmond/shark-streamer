...
export const Profiles = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => setManageProfiles(false), 0);
  }, [dispatch, user?.id]);

  useEffect(() => {
    if (isFirstProfile && user?.id) {
      setTimeout(() => setAddProfilePage(true), 0);
    }
  ...
  return (
    <ProfileContainer>
      { editProfilePage ? (
        ...
        <>
          <PageTitle>{ manageProfiles ? "Manage Profiles" : "Who's Watching?" }</PageTitle>
          <ProfilesRow>
            { user.profiles.map((profile, index) => (
              ...
              >
                <AvatarContainer $isedit={ manageProfiles ? "true" : "false" }>
                  { manageProfiles && (
                    <EditProfileIcon
          ...
          { user.profiles.length > 0 && (
            <div>
              <ManageProfilesButton onClick={ () => setManageProfiles(!manageProfiles) }>
