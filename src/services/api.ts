...
import { Profile, WatchlistItem } from '../store/slices/userSlice';
...
const handleRequest = async (requestFn: () => Promise<any>, errorMessage: string) => {
  try {
...
export const profileAPI = {
  getUserProfiles: (userId: string) => handleRequest(
    () => api.get(`/profiles/user/${userId}`),
    'Failed to get profiles'
  ),

  createProfile: (userId: string, name: string, avatarUrl: string | undefined, isKids: boolean) => handleRequest(
    () => api.post('/profiles', { user_id: userId, name, avatar_url: avatarUrl, is_kids: isKids }),
    'Failed to create profile'
  ),

  updateProfile: (profileId: string, updates: Partial<Profile>) => handleRequest(
    () => api.put(`/profiles/${profileId}`, updates),
    'Failed to update profile'
  ),

  deleteProfile: (profileId: string) => handleRequest(
    () => api.delete(`/profiles/${profileId}`),
    'Failed to delete profile'
  ),

  updateProfileAvatar: (profileId: string, avatarData: string) => handleRequest(
...
export const watchlistAPI = {
  getWatchlist: (profileId: string) => handleRequest(
    () => api.get(`/watchlist/${profileId}`),
    'Failed to get watchlist'
  ),

  addToWatchlist: (profileId: string, movieId: string, movieData: Omit<WatchlistItem, 'id' | 'added_at'>) => handleRequest(
    () => api.post(`/watchlist/${profileId}/${movieId}`, movieData),
    'Failed to add to watchlist'
  ),

  removeFromWatchlist: (profileId: string, movieId: string) => handleRequest(
...
export const historyAPI = {
  getViewingHistory: (profileId: string) => handleRequest(
    () => api.get(`/history/${profileId}`),
    'Failed to get viewing history'
  ),

  addToHistory: (profileId: string, movieData: Omit<WatchlistItem, 'id' | 'added_at'>) => handleRequest(
...
export const subscriptionAPI = {
  updateSubscription: (userId: string, subscriptionPlan: string) => handleRequest(
...
export const movieAPI = {
  ...
  fetchMovieDetails: (movieType: string, movieId: string) => tmdbApi.get(`/${movieType}/${movieId}?api_key=${TMDB_API_KEY}&language=en-US`),
  fetchVideos: (movieType: string, movieId: string) => tmdbApi.get(`/${movieType}/${movieId}/videos?api_key=${TMDB_API_KEY}`),
  fetchImages: (movieType: string, movieId: string) => tmdbApi.get(`/${movieType}/${movieId}/images?api_key=${TMDB_API_KEY}`),
  ...
  fetchCredits: (movieType: string, movieId: string) => tmdbApi.get(`/${movieType}/${movieId}/credits?api_key=${TMDB_API_KEY}`),
  fetchSimilar: async (movieType: string, movieId: string) => {
    ...
    // Filter items to only include those with images AND trailers
    ...
    for (const item of response.data.results) {
      ...
      try {
        // Check if item has videos/trailers
        const videosResponse = await tmdbApi.get(`/${movieType}/${item.id}/videos?api_key=${TMDB_API_KEY}`);
        ...
        // Skip this item if we can't check for videos
      }
    }
